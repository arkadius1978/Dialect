...
const App: React.FC = () => {
  ...
  const initializeGame = (isolationName: string) => {
  ...
  const connectToRoom = (rId: string, pName: string) => {
    ...
    room.onPeerJoin(peerId => {
      ...
      // Se sono già in partita, mando lo stato al nuovo arrivato
      if (gameState.gameStarted) sendState(gameState);
    });
  ...
  const drawCard = (deckKey: keyof GameState['decks']) => {
    ...
    const card = deck.pop()!;
    const myHand = [...(gameState.hands[myPlayerName] || []), card];
  ...
  if (!gameState.gameStarted) {
    return <SetupScreen isolationName={gameState.isolationName} onStart={initializeGame} roomId={roomId} peersCount={peers.length} />;
  }
        ...
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-col">
            ...
            <div className="flex items-center gap-2">
               <span className="text-[9px] uppercase tracking-widest text-stone-400 font-bold">{gameState.isolationName}</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-6">
              {[
                ...
                <div key={d.id} className="flex flex-col items-center">
                  ...
                  >
                    <span className="text-3xl font-black text-stone-600/30">{gameState.decks[d.id as keyof GameState['decks']].length}</span>
      ...
      {selectedCard && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-stone-900/70 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white max-w-sm w-full rounded-2xl shadow-2xl overflow-hidden border-2 border-stone-300 animate-in zoom-in-95">
            ...
            <div className="p-4 bg-stone-50 flex gap-3 border-t border-stone-200">
              {activeTab === 'hand' && (
                <button 
                  onClick={() => playCard(selectedCard)}
                  className="flex-1 bg-red-900 text-white font-bold py-4 rounded-xl hover:bg-red-800 transition-all shadow-lg active:scale-95"
                >
                  Gioca sul Tavolo
                </button>
              )}
              <button 
                ...
                className={`py-4 px-6 rounded-xl font-bold border border-stone-300 text-stone-500 hover:bg-white transition-all ${activeTab !== 'hand' ? 'w-full' : ''}`}
              >
                Chiudi
              </button>
            </div>
...
const SetupScreen: React.FC<{ isolationName: string, roomId: string, peersCount: number, onStart: (name: string) => void }> = ({ isolationName, roomId, peersCount, onStart }) => {
  const [name, setName] = useState(isolationName || '');
  ...
  return (
    <div className="min-h-screen bg-stone-900 flex items-center justify-center p-6">
      <div className="max-w-lg w-full bg-[#f5f2e9] rounded-[2.5rem] border-4 border-stone-800 shadow-2xl p-12 space-y-10 animate-in fade-in duration-700">
        ...
        <div className="space-y-8 bg-white/50 p-8 rounded-3xl border-2 border-stone-100 shadow-inner">
           <div className="space-y-4">
              <label className="text-[10px] font-bold uppercase tracking-widest text-stone-500 font-cinzel block ml-1 text-center">Definisci l'Isolamento</label>
              <input 
                placeholder="Es: L'Avamposto 31..."
                value={name}
                onChange={e => setName(e.target.value)}
                className="w-full bg-white border-2 border-stone-100 p-5 rounded-2xl font-cinzel text-2xl text-center outline-none focus:border-red-900 transition-all"
              />
           </div>
           
           <div className="text-center space-y-2">
              <p className="text-xs text-stone-500 leading-relaxed italic">
                "Una volta iniziato, il mazzo sarà mescolato e sincronizzato con tutti gli altri partecipanti."
              </p>
           </div>
        </div>
